...
export async function getBundles(request: Request): Promise<Bundle[]> {
...
export async function getBundleById(request: Request, id: string): Promise<Bundle | null> {
  const { data, error } = await supabase
    .from("bundles")
    .select(`
      *,
...
export async function getCourseById(request: Request, id: string) {
  const { data, error } = await supabase
    .from("courses")
    .select("*")
    .eq("id", id)
...
export async function getStudentEnrollments(request: Request, studentId: string): Promise<Enrollment[]> {
  const { data, error } = await supabase
    .from("enrollments")
    .select(`
      *,
...
export async function getPaymentMethods(request: Request): Promise<PaymentMethod[]> {
  const { data, error } = await supabase
...
export async function submitPayment(
  ...
  if (data.screenshot_urls.length !== 1) {
    throw new Error("Please upload exactly one payment screenshot.");
  }
...
export async function getAllPaymentSubmissions(request: Request): Promise<any[]> {
  const { data, error } = await supabase
    .from("payment_submissions")
    .select(`
      *,
...
export async function updatePaymentStatus(
  ...
  // If approved, update enrollment status with 6-month expiration and record teacher earning
  if (status === 'approved' && submission) {
    ...
    // Record teacher earning if it's a course enrollment
    const enrollment = submission.enrollment as any;
    if (enrollment?.course_id && enrollment?.course?.[0]?.teacher_id) {
      const teacherShare = Number(submission.amount) * 0.7; // 70% to teacher
      
      await supabase
        .from('teacher_earnings')
        .insert({
          teacher_id: enrollment.course[0].teacher_id,
