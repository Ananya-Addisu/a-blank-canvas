...
export async function getQuizzesByTeacher(teacherId: string) {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      ...
      attempts:quiz_attempts(id, score, status)
    `)
  ...
  return (data || []).map(quiz => ({
    ...
    attempt_count: quiz.attempts?.length || 0,
    average_score: quiz.attempts?.length > 0
...
export async function createQuiz(teacherId: string, quizData: {
  title: string;
  description?: string;
  courseId?: string;
  duration?: number;
...
export async function updateQuiz(quizId: string, quizData: Partial<{
  title: string;
  description: string;
  duration: number;
  passingScore: number;
  ...
  videoUrl: string;
  feedbackMode: 'after_submit' | 'immediate';
}>) {
...
export async function addQuizQuestion(quizId: string, questionData: {
  questionText: string;
...
export async function updateQuizQuestion(questionId: string, questionData: Partial<{
  questionText: string;
  options: Record<string, string>;
  correctAnswer: string;
  points: number;
...
export async function getStudentQuizAttempts(studentId: string, quizId?: string) {
  let query = supabase
    ...
    .select(`
      *,
      quiz:quizzes(id, title, passing_score, course:courses(id, name))
    `)
