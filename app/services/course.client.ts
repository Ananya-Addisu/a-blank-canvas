...
export async function getAllCourses() {
  const { data, error } = await supabase
    .from('courses')
    .select(`*, teacher:teachers(id, full_name, specialization), bundle_courses(bundle:bundles(id, name, year_level, semester))`)
    .eq('status', 'active')
...
export async function getCourseById(courseId: string) {
  const { data, error } = await supabase
    .from('courses')
    .select(`*, teacher:teachers(id, full_name, email, specialization, bio), chapters:chapters(id, title, description, order_index, is_published, lessons:lessons(id, title, description, lesson_type, content_url, youtube_url, video_source, download_url, is_downloadable, duration, page_count, order_index, is_published, is_preview, approval_status))`)
    .eq('id', courseId)
    .single();
  ...
  if (data?.chapters) {
    ...
    data.chapters.forEach((chapter: any) => {
      if (chapter.lessons) {
        chapter.lessons = chapter.lessons.filter((lesson: any) => lesson.approval_status === 'approved' || lesson.is_preview);
...
export async function getCoursesByTeacher(teacherId: string) {
  const { data, error } = await supabase
    .from('courses')
    .select('*, chapters:chapters(id), enrollments:enrollments(id, student_id)')
...
export async function createCourse(teacherId: string, courseData: { name: string; description: string; category: string; department: string; price: number; discountPercentage?: number; thumbnailUrl?: string; syllabus?: string; }) {
  const { data, error } = await supabase.from('courses').insert({ teacher_id: teacherId, name: courseData.name, description: courseData.description, category: courseData.category, department: courseData.department, price: courseData.price, discount_percentage: courseData.discountPercentage || 0, thumbnail_url: courseData.thumbnailUrl, syllabus: courseData.syllabus, status: 'active' }).select().single();
  if (error) return { success: false, error: error.message };
  return { success: true, data };
}

export async function updateCourse(courseId: string, courseData: Partial<{ name: string; description: string; category: string; department: string; price: number; discountPercentage: number; thumbnailUrl: string; syllabus: string; status: string; }>) {
  ...
  const { data, error } = await supabase.from('courses').update(updateData).eq('id', courseId).select().single();
  if (error) return { success: false, error: error.message };
  return { success: true, data };
}
...
export async function createChapter(courseId: string, chapterData: { title: string; description?: string; orderIndex: number; }) {
  const { data, error } = await supabase.from('chapters').insert({ course_id: courseId, title: chapterData.title, description: chapterData.description, order_index: chapterData.orderIndex, is_published: false }).select().single();
  if (error) return { success: false, error: error.message };
  return { success: true, data };
}

export async function updateChapter(chapterId: string, chapterData: Partial<{ title: string; description: string; orderIndex: number; isPublished: boolean; }>) {
  const updateData: any = {};
  ...
  const { data, error } = await supabase.from('chapters').update(updateData).eq('id', chapterId).select().single();
  if (error) return { success: false, error: error.message };
  return { success: true, data };
}
...
export async function createLesson(chapterId: string, lessonData: { title: string; description?: string; lessonType: 'video' | 'pdf' | 'quiz'; contentUrl?: string; youtubeUrl?: string; videoSource?: 'upload' | 'youtube'; duration?: number; pageCount?: number; orderIndex: number; isPreview?: boolean; }) {
  ...
  const { data, error } = await supabase.from('lessons').insert({
    chapter_id: chapterId, title: lessonData.title, description: lessonData.description,
    lesson_type: lessonData.lessonType, content_url: lessonData.videoSource === 'upload' ? lessonData.contentUrl : null,
    youtube_url: youtubeData?.embedUrl || null, video_source: lessonData.videoSource || 'upload',
    duration: lessonData.duration, page_count: lessonData.pageCount, order_index: lessonData.orderIndex,
    is_preview: lessonData.isPreview || false, is_published: false, approval_status: 'pending',
  }).select().single();
...
export async function updateLesson(lessonId: string, lessonData: Partial<{ title: string; description: string; contentUrl: string; youtubeUrl: string; videoSource: 'upload' | 'youtube'; duration: number; pageCount: number; orderIndex: number; isPublished: boolean; isPreview: boolean; }>) {
  const updateData: any = {};
