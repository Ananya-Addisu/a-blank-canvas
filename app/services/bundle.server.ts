...
export async function getAllBundles() {
  ...
  return (data || []).map(bundle => ({
    ...bundle,
    courses: bundle.bundle_courses?.map((bc: any) => bc.course) || [],
    course_count: bundle.bundle_courses?.length || 0
  }));
}
...
export async function getBundleById(bundleId: string) {
      ...
      bundle_courses:bundle_courses(
        course:courses(
          ...
          chapters:chapters(
            ...
            description,
            order_index,
            lessons:lessons(id, title, lesson_type)
          )
        )
      )
    `)
  ...
  const totalLessons = courses.reduce((sum: number, course: any) => {
    const lessonCount = course.chapters?.reduce(
      (chapterSum: number, chapter: any) => chapterSum + (chapter.lessons?.length || 0),
      0
    ) || 0;
...
export async function createBundle(bundleData: {
  ...
  const { data: bundle, error: bundleError } = await supabase
    ...
    .insert({
      ...
      semester: bundleData.semester,
      year_level: bundleData.yearLevel,
      price: bundleData.price,
      discount_percentage: bundleData.discountPercentage || 0,
      thumbnail_url: bundleData.thumbnailUrl,
      is_active: true
    })
    .select()
    .single();
  ...
  if (bundleData.courseIds && bundleData.courseIds.length > 0) {
    const bundleCourses = bundleData.courseIds.map(courseId => ({
      bundle_id: bundle.id,
      course_id: courseId
    }));
    ...
    if (coursesError) {
      console.error('Error adding courses to bundle:', coursesError);
...
export async function updateBundle(bundleId: string, bundleData: Partial<{
  name: string;
  description: string;
  semester: string;
  yearLevel: string;
  ...
  if (bundleData.discountPercentage !== undefined) {
    updateData.discount_percentage = bundleData.discountPercentage;
  }
...
export async function getBundlesBySemester(semester: string) {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      bundle_courses:bundle_courses(
        course:courses(id, name, thumbnail_url)
      )
    `)
  ...
  if (error) {
    console.error('Error fetching bundles by semester:', error);
    return [];
  }
...
export async function getBundlesByYearLevel(yearLevel: string) {
  ...
  if (error) {
    console.error('Error fetching bundles by year level:', error);
    return [];
  }
...
// Calculate actual bundle price with discount
