...
// FIX: Dark/light mode preference is persisted locally via @dazl/color-scheme which saves to localStorage automatically
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={triggerText ? `Theme: ${label}` : "Toggle color scheme"}
          className={triggerText ? style.triggerButton : undefined}
        >
          <span className={style.triggerIcon}>{icon}</span>
          {triggerText && <span className={style.triggerLabel}>Theme · {label}</span>}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent>
        <DropdownMenuItem
